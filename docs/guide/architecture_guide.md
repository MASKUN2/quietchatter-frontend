...
- **Use Custom Hooks**: Complex data fetching logic (e.g., infinite scroll, debouncing searches, pagination) should be wrapped in individual custom hooks within the `src/hooks/` folder. **Page-level data logic (fetching + state + side effects) must always live in a dedicated hook**, not inside the page component itself (see `useBookDetail.ts`, `useMyTalks.ts` as examples).
- **useCallback for Hook Handlers**: Any function returned from a custom hook (e.g., `onSubmit`, `onReaction`, `onUpdate`) **must** be wrapped in `useCallback` with a proper dependency array. This prevents unnecessary re-renders in child components that receive these functions as props.
- **Error Messages via Constants**: All user-facing error and success strings **must** be defined in `src/constants/index.ts` under the `MESSAGES` object. Never hardcode strings like `'삭제에 실패했습니다.'` directly in component files. This keeps all copy centralized and easy to update.
- **Error Handling**: When a background network request fails, always provide appropriate feedback to the user using the global Toast system. **Never use native browser `alert()` popups**. Instead, import `useToast` from `@/hooks/useToast` and call `showToast('message', 'error')`. For business logic errors, usually show the message sent from the backend as it is. **Never create a local `<Snackbar>` inside a component for toasts** — always use `useToast`.
